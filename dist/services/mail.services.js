...
const emailConfiguration_1 = __importDefault(require("../utils/emailConfiguration"));
const date_fns_1 = require("date-fns");
const sendLeavePlanEmail = (body) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const fromMailId = process.env.EMAIL_FROM;
    const bccList = ((_a = process.env.MANAGER_EMAIL) === null || _a === void 0 ? void 0 : _a.split(",").map((email) => email.trim())) || [];
    const { start_date, buffer } = body;
    const start = new Date(start_date);
    const months = (0, date_fns_1.format)(start, "MMMM");
    const year = start === null || start === void 0 ? void 0 : start.getFullYear();
    try {
      ...
      <html lang="en">
        ...
        yield emailConfiguration_1.default.transporter.sendMail({
            from: `${fromMailId}`,
            to: bccList,
...
const sendLeavePlanUpdateReminder = (body) => __awaiter(void 0, void 0, void 0, function* () {
    const fromMailId = process.env.EMAIL_FROM;
    const { bccList } = body;
    try {
        const template = `<!DOCTYPE html>
...
const sendManagerLeaveApprovalReminder = (body) => __awaiter(void 0, void 0, void 0, function* () {
    const { bccList } = body;
    ...
    try {
      ...
      <html lang="en">
        ...
        console.log("Manager leave plan application and approval reminder sent successfully.");
        return {
            message: "Manager leave plan application and approval reminder sent successfully",
            status: true,
        };
    }
    catch (error) {
        console.error("Error sending manager leave plan application and approval reminder:", error);
        return {
            message: "Error sending manager leave plan application and approval reminder",
            status: false,
        };
    }
});
